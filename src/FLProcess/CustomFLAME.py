...
from flwr.server.strategy.aggregate import aggregate_krum, _compute_distances, aggregate
...
class CustomFLAME(FedAvg):
    ...
    def __init__(
        ...
        self.best_cilent_list = best_client_list
        self.best_client_ids = best_client_ids
    ...
    def aggregate_fit(
        ...
        weights, indices = aggregate_krum_custom(
            weights_results, self.num_malicious_clients, self.num_clients_to_keep, is_removing_poisoners=False
        )
        print(indices)
        self.best_client_ids.append(indices)
...
def aggregate_krum_custom(
    ...
    if to_keep > 0:
        ...
        if is_removing_poisoners:
            return aggregate(best_results), best_indices
        else:
            return aggregate(results), best_indices

    if is_removing_poisoners:
        # Return the model parameters that minimize the score (Krum)
        return weights[np.argmin(scores)], scores
    else:
        return aggregate(results), scores
